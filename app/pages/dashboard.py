...
[data-testid="stMetric"] label { color: #8b949e !important; font-family: 'JetBrains Mono', monospace; font-size: 11px !important; text-transform: uppercase; }
[data-testid="stMetric"] [data-testid="stMetricValue"] { color: #00ff88 !important; font-family: 'JetBrains Mono', monospace; font-size: 28px !important; font-weight: 700; }
</style>
...
with col_info:
    st.markdown("""
    <div style='font-size:11px;color:#8b949e;font-family:JetBrains Mono;padding:8px 0;'>
        Metrics: Relevancy (50%) + Faithfulness (30%) + CWE Accuracy (20%) | Pass threshold: 0.60
    </div>
    """, unsafe_allow_html=True)
...
if not stats:
    st.markdown("""
    <div style='text-align:center;padding:60px;color:#8b949e;font-family:JetBrains Mono;font-size:13px;'>
...
with chart_col:
    ...
    fig.add_trace(go.Scatter(
        ...
        name="Relevancy", line=dict(color="#00ff88", width=2),
        mode="lines+markers"
    ))
    fig.add_trace(go.Scatter(
        x=df["run_timestamp"], y=df["faithfulness_score"],
        name="Faithfulness", line=dict(color="#00c9ff", width=2),
        mode="lines+markers"
    ))
    fig.add_trace(go.Scatter(
    ...
    st.markdown("""
    <div style='font-family:JetBrains Mono;font-size:11px;color:#8b949e;
                text-transform:uppercase;letter-spacing:2px;margin:16px 0 8px 0;'>
        Per-Finding Overall Score
    </div>
    """, unsafe_allow_html=True)
    ...
    fig2 = px.bar(
        ...
        y="overall_score",
        color="passed_label",
        color_discrete_map={"Pass": "#00ff88", "Fail": "#ff4444"},
...
with table_col:
    ...
    if "llm_model" in df.columns:
        ...
        for _, row in model_stats.iterrows():
            color = "#00ff88" if row["pass_rate"] >= 50 else "#ff4444"
            st.markdown(f"""
            <div style='background:#0d1117;border:1px solid #1e2d40;border-left:3px solid {color};
                        border-radius:6px;padding:10px 12px;margin-bottom:8px;'>
                <div style='font-family:JetBrains Mono;font-size:11px;color:{color};font-weight:700;'>
                    {row['llm_model']}
                </div>
                <div style='font-size:10px;color:#8b949e;margin-top:4px;line-height:1.8;'>
